...
COMMENT ON COLUMN CLIENT.client_profile_url IS 'profile_url';
...
CREATE TABLE PEREPARED_RECIPE_COMMENT_REACTION
(
...
CREATE SEQUENCE SEQ_PEREPARED_RECIPE_COMMENT_REACTION
START WITH 1
INCREMENT BY 1;

CREATE OR REPLACE TRIGGER SEQ_TRG_PEREPARED_RECIPE_COMMENT_REACTION
BEFORE INSERT ON PEREPARED_RECIPE_COMMENT_REACTION
REFERENCING NEW AS NEW FOR EACH ROW
BEGIN
  SELECT SEQ_PEREPARED_RECIPE_COMMENT_REACTION.NEXTVAL
  INTO: NEW.comment_id
  FROM DUAL;
END;

COMMENT ON COLUMN PEREPARED_RECIPE_COMMENT_REACTION.comment_id IS '댓글고유번호';

COMMENT ON COLUMN PEREPARED_RECIPE_COMMENT_REACTION.user_email IS '유저이메일';

COMMENT ON COLUMN PEREPARED_RECIPE_COMMENT_REACTION.reaction IS '좋아요/싫어요';
...
COMMENT ON COLUMN USER_DATA.user_profile_url IS '프로필사진';
...
ALTER TABLE PEREPARED_RECIPE_COMMENT_REACTION
  ADD CONSTRAINT FK_PREPARED_RECIPE_COMMENT_TO_PEREPARED_RECIPE_COMMENT_REACTION
    FOREIGN KEY (comment_id)
    REFERENCES PREPARED_RECIPE_COMMENT (prepared_recipe_comment_id);

ALTER TABLE PEREPARED_RECIPE_COMMENT_REACTION
  ADD CONSTRAINT FK_USER_DATA_TO_PEREPARED_RECIPE_COMMENT_REACTION
    FOREIGN KEY (user_email)
    REFERENCES USER_DATA (user_email);
